...
`INSTALL-WINDOWS.cmd`. The assistant detects Mixxx and the current stable
version, then offers a safe parallel installation or an advanced replacement
with a backup and explicit confirmation. GitHub Actions builds the artefact
from the same base and the same ten patches; consult
